...
# 14 "keypad_interface.c" 2
# 33 "keypad_interface.c"
void init();
...
void main() {
    ...
    while (1) {
        key = get_key();
        if (key != 'n') {
            Lcd_set_cursor(1, cursor_position);
            Lcd_data(key);
            cursor_position++;
...
unsigned char get_key() {
    ...
    if (!(PORTD & 0x10)) { debounce(); return '1'; }
    if (!(PORTD & 0x20)) { debounce(); return '2'; }
    if (!(PORTD & 0x40)) { debounce(); return '3'; }


    PORTB = (PORTB & 0xF0) | 0x02;
    if (!(PORTD & 0x10)) { debounce(); return '4'; }
    if (!(PORTD & 0x20)) { debounce(); return '5'; }
    if (!(PORTD & 0x40)) { debounce(); return '6'; }


    PORTB = (PORTB & 0xF0) | 0x04;
    if (!(PORTD & 0x10)) { debounce(); return '7'; }
    if (!(PORTD & 0x20)) { debounce(); return '8'; }
    if (!(PORTD & 0x40)) { debounce(); return '9'; }


    PORTB = (PORTB & 0xF0) | 0x08;
    if (!(PORTD & 0x10)) { debounce(); return '*'; }
    if (!(PORTD & 0x20)) { debounce(); return '0'; }
    if (!(PORTD & 0x40)) { debounce(); return '#'; }
...
void debounce() {
    _delay((unsigned long)((20)*(6000000/4000.0)));
    while (!(PORTD & 0x10) || !(PORTD & 0x20) || !(PORTD & 0x40));
    _delay((unsigned long)((20)*(6000000/4000.0)));
}
