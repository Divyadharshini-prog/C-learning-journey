...
void Lcd_command(unsigned char i)
{
    ...
    PORTC |= 0X01;
    PORTC &= ~0X01;
    _delay((unsigned long)((2)*(6000000/4000.0)));
}
...
void Lcd_output(float voltage)
{
    unsigned int i = (unsigned int)(voltage * 10);
...
void display_status(void)
{
    ...
    if (battery_voltage < 17.5)
    {
        for (x = 0; x < 12; x++)
        {
            Lcd_data("BATTERY LOW"[x]);
        }
    }
    else if (battery_voltage <= 20.5)
    {
        for (x = 0; x < 14; x++)
        {
            Lcd_data("BATTERY NORMAL"[x]);
        }
    }
    else
    {
