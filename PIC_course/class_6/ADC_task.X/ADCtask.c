...
#include <stdint.h>
#define CALIBRATION_VALUE   100  // New calibration value for 0 to 100 range
...
void main()
{
    ...
    while(1) 
    {
        ADCON0 = 0x91;   // Select Channel 2 (RA2) and turn on ADC
        ...
        //  typecasting to ensure the calculation
        volt = (unsigned char)(((unsigned long)result * CALIBRATION_VALUE) / NUMBER_OF_BIT);
        ...
        lcd_data('V');  // Append 'V' to the calibrated value
...
void lcd_output(unsigned int i)
{
    ...
    while (i != 0) {
        k[j++] = (i % 10) + '0'; // Convert digit to character
        i /= 10;
    }
...
void init()
{
    TRISA = 0x04;  // Set RA2 as input (0x04 = 0000 0100)
    TRISC = 0x00;
    TRISD = 0x00;
    ADCON0 = 0x91;  // Select Channel 2 (RA2) and turn on ADC
    ADCON1 = 0x8E;
